...
async def test_timer(dut):
    ...
    PWM_PERIOD = CPS // 2
    cocotb.log.info(
        f"CYCLES_PER_SECOND={CPS}  HOLD={HOLD}  SHORT={SHORT}  PWM_PERIOD={PWM_PERIOD}"
    )
    ...
    await tick_n(dut, 3)
    assert int(dut.blank_seconds.value) == 0, (
        "blank_seconds must be 0 after exiting edit"
    )
    assert int(dut.blank_minutes.value) == 0, (
        "blank_minutes must be 0 after exiting edit"
    )
    ...
    assert int(dut.seconds_disp.value) == 3, (
        "seconds must still be 3 after exiting edit"
    )
...
async def test_countdown_delay_and_period(dut):
    ...
    await tick_n(dut, 3)
    assert int(dut.seconds_disp.value) == 3, (
        "setup: seconds must be 3 before timing test"
    )
